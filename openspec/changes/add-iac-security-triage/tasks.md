...
- [ ] 3.1 **Human task.** Triage all 12 first-party alerts in GitHub code scanning:
      dismiss with a comment carrying the rationale and the repo-relative paths of any
      ADRs or design docs relied on, or leave open for promotion to an issue. Judge each
      finding independently rather than deciding once per resource; verify all 12 alerts
      have a recorded outcome and a non-empty comment
- [ ] 3.2 Implement the fixture export: read alert state via `gh api
      ...
      key; verify every key resolves to a first-party finding in the baseline fixture and
      that the export predates any triage run
...
- [ ] 5.1 Score a full triage run against the fixture; record per-rule agreement in
      ...
      including that 8 of the 10 first-party rules fire exactly once
...
- [ ] 6.2 Implement idempotency keyed on the finding key; verify a second run with unchanged
...
- [ ] 7.1 Add an explicit, reviewable allowlist of rule IDs permitted autonomous
      ...
      current corpus the allowlist is empty, since the largest first-party rule is n=2
